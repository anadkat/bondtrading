...
from .models import Bond, Order, OrderRequest, OrderResponse, HistoricalPrices, OrderBook
from .services.moment_api import MomentAPIService
from .storage import BondStorage
...
app.add_middleware(
    CORSMiddleware,
    allow_origins=["http://localhost:3000", "http://localhost:4000", "http://localhost:5000"],
...
async def serve_frontend():
    ...
    frontend_path = os.path.join(os.path.dirname(__file__), "../../dist/public/index.html")
    if os.path.exists(frontend_path):
        return FileResponse(frontend_path)
    return {"message": "Bond Screener API is running! Frontend not built yet."}
